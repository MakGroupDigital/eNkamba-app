 ...
 */
export const searchUserByIdentifier = functions.https.onCall(
  async (data: {
    searchMethod: 'card' | 'account' | 'email' | 'phone';
    searchQuery: string;
    ...
    if (context.auth.uid !== currentUserId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Vous ne pouvez rechercher que pour votre propre compte'
      );
    }
 ...
 */
export const createMoneyRequest = functions.https.onCall(
  async (data: {
    fromUserId: string;
    toUserId: string;
    amount: number;
    ...
    if (context.auth.uid !== fromUserId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Vous ne pouvez créer que vos propres demandes'
      );
    }
    ...
    if (fromUserId === toUserId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Vous ne pouvez pas demander de l\'argent à vous-même'
      );
    }
 ...
 */
export const acceptMoneyRequest = functions.https.onCall(
  async (data: {
    requestId: string;
    userId: string;
  }, context) => {
 ...
 */
export const rejectMoneyRequest = functions.https.onCall(
  async (data: {
    requestId: string;
    userId: string;
    reason?: string;
 ...
 */
export const getMoneyRequests = functions.https.onCall(
  async (data: {
    userId: string;
    type: 'received' | 'sent';
  }, context) => {
