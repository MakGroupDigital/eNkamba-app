...
exports.getCollectionStats = exports.createContactPaymentCode = exports.getPaymentLinkDetails = exports.processPaymentLink = exports.createPaymentLink = void 0;
...
exports.createPaymentLink = functions.https.onCall(async (data, context) => {
    ...
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez créer que vos propres liens');
    }
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la création du lien');
    }
});
/**
 * Cloud Function pour valider et traiter un paiement via lien
 */
exports.processPaymentLink = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!linkDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Lien de paiement non trouvé');
        }
        ...
        if (linkData.status !== 'active') {
            throw new functions.https.HttpsError('failed-precondition', 'Ce lien de paiement n\'est plus actif');
        }
        // Vérifier l'expiration
        if (new Date(linkData.expiresAt) < new Date()) {
            throw new functions.https.HttpsError('failed-precondition', 'Ce lien de paiement a expiré');
        }
        // Vérifier le montant si spécifié
        if (linkData.amount && amount !== linkData.amount) {
            throw new functions.https.HttpsError('invalid-argument', `Le montant doit être ${linkData.amount} CDF`);
        }
 ...
 */
exports.getPaymentLinkDetails = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la récupération du lien');
    }
});
/**
 * Cloud Function pour créer un code de paiement par contact
 */
exports.createContactPaymentCode = functions.https.onCall(async (data, context) => {
    const { userId, amount } = data;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez créer que vos propres codes');
    }
    try {
        const db = admin.firestore();
        // Générer un code unique
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la création du code');
    }
});
/**
 * Cloud Function pour obtenir les statistiques d'encaissement
 */
exports.getCollectionStats = functions.https.onCall(async (data, context) => {
    const { userId } = data;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez voir que vos propres statistiques');
    }
    try {
        const db = admin.firestore();
