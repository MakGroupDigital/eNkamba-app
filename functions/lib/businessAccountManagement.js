...
exports.getPendingBusinessRequests = exports.rejectBusinessRequest = exports.approveBusinessRequest = exports.submitBusinessRequest = void 0;
 ...
 */
exports.submitBusinessRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!data.businessName?.trim()) {
        throw new functions.https.HttpsError('invalid-argument', 'Le nom de l\'entreprise est requis');
    }
    if (!['COMMERCE', 'LOGISTICS', 'PAYMENT'].includes(data.type)) {
        throw new functions.https.HttpsError('invalid-argument', 'Type d\'entreprise invalide');
    }
    if (!data.registrationNumber?.trim()) {
        throw new functions.https.HttpsError('invalid-argument', 'Le numéro d\'enregistrement est requis');
    }
    try {
        const now = Date.now();
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la soumission de la demande');
    }
});
/**
 * Approuver une demande de compte entreprise (Admin)
 */
exports.approveBusinessRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    ...
    if (!adminDoc.exists) {
        throw new functions.https.HttpsError('permission-denied', 'Vous n\'avez pas les permissions pour approuver les demandes');
    }
    try {
 ...
 */
exports.rejectBusinessRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    // Vérifier que l'utilisateur est admin
    const adminDoc = await db.collection('admins').doc(context.auth.uid).get();
    if (!adminDoc.exists) {
        throw new functions.https.HttpsError('permission-denied', 'Vous n\'avez pas les permissions pour rejeter les demandes');
    }
    try {
        const requestRef = db.collection('business_requests').doc(data.requestId);
        const requestDoc = await requestRef.get();
 ...
 */
exports.getPendingBusinessRequests = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié');
    }
    // Vérifier que l'utilisateur est admin
    const adminDoc = await db.collection('admins').doc(context.auth.uid).get();
    if (!adminDoc.exists) {
        throw new functions.https.HttpsError('permission-denied', 'Vous n\'avez pas les permissions');
    }
    try {
...
//# sourceMappingURL=businessAccountManagement.js.map