...
exports.cancelTransaction = exports.getTransactionHistory = exports.getWalletBalance = exports.withdrawFundsFromWallet = exports.addFundsToWallet = void 0;
...
exports.addFundsToWallet = functions.https.onCall(async (data, context) => {
    ...
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez modifier que votre propre portefeuille');
    }
    // Valider le montant
    if (!amount || amount <= 0) {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de l\'ajout de fonds');
    }
});
/**
 * Cloud Function pour retirer des fonds du portefeuille
 */
exports.withdrawFundsFromWallet = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors du retrait');
    }
});
/**
...
exports.getWalletBalance = functions.https.onCall(async (data, context) => {
    ...
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez voir que votre propre solde');
    }
    try {
        const db = admin.firestore();
        const userRef = db.collection('users').doc(userId);
...
exports.cancelTransaction = functions.https.onCall(async (data, context) => {
    ...
    if (context.auth.uid !== userId) {
        throw new functions.https.HttpsError('permission-denied', 'Vous ne pouvez annuler que vos propres transactions');
    }
    try {
        const db = admin.firestore();
        ...
        if (diffHours > 24) {
            throw new functions.https.HttpsError('failed-precondition', 'Vous ne pouvez annuler que les transactions de moins de 24h');
        }
        // Vérifier que la transaction n'est pas déjà annulée
        if (transaction.status === 'cancelled') {
            throw new functions.https.HttpsError('failed-precondition', 'Cette transaction est déjà annulée');
        }
        // Vérifier que la transaction est complétée
        if (transaction.status !== 'completed') {
            throw new functions.https.HttpsError('failed-precondition', 'Seules les transactions complétées peuvent être annulées');
        }
