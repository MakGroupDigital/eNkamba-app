...
exports.addFunds = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!userId || !amount || !paymentMethod) {
            throw new functions.https.HttpsError('invalid-argument', 'Paramètres manquants: userId, amount, paymentMethod requis');
        }
        if (amount <= 0) {
        ...
        const transactionData = {
            ...
            newBalance,
            description: `Ajout de fonds via ${paymentMethod === 'mobile_money'
                ? 'Mobile Money'
                : paymentMethod === 'card'
                    ? 'Carte bancaire'
                    : 'Virement bancaire'}`,
        ...
        throw new functions.https.HttpsError('internal', error.message || 'Erreur lors de l\'ajout de fonds');
    }
});
