...
const ReferralGiftIcon = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <rect x="10" y="19" width="28" height="20" rx="6" fill="#25543A" />
    ...
    <circle cx="36" cy="14" r="4" fill="#25543A" />
  </svg>
);

const NetworkRewardIcon = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <circle cx="24" cy="14" r="6" fill="#25543A" />
    <circle cx="14" cy="32" r="5" fill="#25543A" />
    <circle cx="34" cy="32" r="5" fill="#FFB545" />
    <path d="M21 19l-5 8M27 19l5 8M19 32h10" stroke="#25543A" strokeWidth="3" strokeLinecap="round" opacity="0.65" />
  </svg>
);
    ...
    <div className="container mx-auto max-w-4xl p-3 space-y-4 animate-in fade-in duration-500 sm:p-4">
      <header className="overflow-hidden rounded-2xl bg-gradient-to-br from-[#25543A] to-[#25543A] p-4 text-white shadow-lg shadow-[#25543A]/20">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button
            className="hidden h-10 shrink-0 rounded-xl bg-white px-3 text-xs font-bold text-[#25543A] hover:bg-white/90 sm:inline-flex"
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card className="overflow-hidden border-[#25543A]/10 bg-white shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-bold text-[#25543A]">Amis Parrainés</CardTitle>
            <Users className="h-4 w-4 text-[#25543A]" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-black text-[#25543A]">Non disponible</div>
            <p className="text-xs text-muted-foreground">Aucune source réelle de filleuls connectée.</p>
          </CardContent>
        </Card>
        <Card className="overflow-hidden border-[#25543A]/10 bg-white shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-bold text-[#25543A]">Gains Totaux</CardTitle>
        ...
        <CardContent className="space-y-4 p-4">
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <Input id="referral-code" value={isLoading ? 'Chargement...' : referralCode || ''} readOnly className="h-12 rounded-xl border-[#25543A]/20 bg-[#f7faf8] font-mono text-lg font-black" />
              <Button size="icon" className="h-12 w-12 rounded-xl bg-[#25543A] hover:bg-[#25543A]" disabled={!referralCode} onClick={() => handleCopy(referralCode || '', "Votre code a été copié.")}>
           ...
           <div>
            ...
            <div className="flex gap-2 mt-1">
              <Input id="referral-link" value={referralLink} readOnly className="h-12 rounded-xl border-[#25543A]/20 bg-[#f7faf8]" />
              <Button size="icon" className="h-12 w-12 rounded-xl bg-[#25543A] hover:bg-[#25543A]" disabled={!referralLink} onClick={() => handleCopy(referralLink, "Votre lien de partage a été copié.")}>
                <Copy className="h-4 w-4" />
              </Button>
            </div>
          </div>
        </CardContent>
        <CardFooter className="flex flex-col items-center gap-2 border-t border-[#25543A]/10 px-4 py-3 sm:flex-row">
            <Button className="w-full rounded-xl bg-[#25543A] hover:bg-[#25543A] sm:w-auto" onClick={() => handleShare('whatsapp')}><WhatsAppIcon/> Partager sur WhatsApp</Button>
            <Button className="w-full rounded-xl border-[#25543A]/20 text-[#25543A] hover:bg-[#25543A]/5 sm:w-auto" variant="outline" onClick={() => handleShare('telegram')}><TelegramIcon/> Partager sur Telegram</Button>
            <Button className="w-full rounded-xl border-[#25543A]/20 text-[#25543A] hover:bg-[#25543A]/5 sm:w-auto" variant="outline" onClick={() => handleShare('email')}><Mail className="h-4 w-4"/> Partager par e-mail</Button>
        </CardFooter>
      </Card>
      ...
      <Card className="overflow-hidden border-[#25543A]/10 bg-white shadow-sm">
        <CardHeader className="border-b border-[#25543A]/10 px-4 py-3">
          <CardTitle className="font-headline flex items-center gap-2 text-lg text-[#25543A]">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#25543A]/10">
              <QrCode className="h-5 w-5 text-[#25543A]" />
            </span>
            Partage par QR Code
          </CardTitle>
        ...
        <CardContent className="flex flex-col items-center justify-center gap-4 p-4 text-center">
          {qrCodeDataUrl ? (
            <img src={qrCodeDataUrl} alt="QR Code de parrainage" className="rounded-2xl border border-[#25543A]/20 bg-white p-2 shadow-sm" />
          ) : (
            <div className="flex h-[200px] w-[200px] items-center justify-center rounded-2xl border-2 border-dashed border-[#25543A]/25 bg-[#f7faf8]">
              <p className="text-sm text-muted-foreground">Cliquez pour générer</p>
            </div>
          )}
          <Button onClick={generateQrCode} disabled={isLoadingQr || !referralLink} className="rounded-xl bg-[#25543A] hover:bg-[#25543A]">
            {isLoadingQr ? <Loader2 className="animate-spin" /> : (qrCodeDataUrl ? "Regénérer le QR Code" : "Générer le QR Code")}
          </Button>
          <div className="flex items-center gap-2 rounded-full bg-[#25543A]/10 px-3 py-1 text-xs font-semibold text-[#25543A]">
            <ShieldCheck className="h-3.5 w-3.5 text-[#25543A]" />
