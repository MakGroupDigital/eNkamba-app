...
const Logo = () => (
  <div className="flex flex-col items-center mb-10 mt-8">
    <div className="relative w-16 h-16 mb-4 group cursor-pointer">
      <div className="absolute inset-0 bg-[#25543A] opacity-20 blur-xl rounded-full group-hover:opacity-40 transition-opacity"></div>
      <svg viewBox="0 0 100 100" className="w-full h-full relative z-10">
        <path d="M25 80 L25 30 Q25 20 35 25 L65 55 Q75 60 75 50 L75 20" stroke="#25543A" strokeWidth="14" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M75 20 L55 20 M75 20 L75 40" stroke="#25543A" strokeWidth="14" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="88" cy="15" r="4" fill="#FFCC00" className="animate-pulse" />
      </svg>
    </div>
    <div className="font-brand font-bold text-xl tracking-wide">
      eNkamba<span className="font-light text-[#25543A]">.ai</span>
    </div>
  </div>
);
        ...
        {menuItems.map((item: any) => (
          <button
            ...
            className={`flex items-center gap-3 p-4 rounded-xl font-medium transition-all duration-200 ${
              activeTab === item.id
                ? 'bg-[#25543A] text-white shadow-lg shadow-[#25543A]/20'
...
const ChatInterface = ({ chatId }: { chatId: string }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#25543A] relative overflow-hidden">
      <div className="ai-glow"></div>
      <header className="h-20 flex items-center justify-between px-8 border-b border-white/5 bg-[#25543A]/80 backdrop-blur-md z-10">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-[#25543A]/10 flex items-center justify-center border border-[#25543A]/20">
            <MessageSquare size={18} className="text-[#25543A]" />
          </div>
          <div>
            <h1 className="font-brand font-bold text-lg">Assistant eNkamba</h1>
            <span className="text-[10px] text-[#25543A] uppercase font-bold tracking-widest">Actif</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="w-8 h-8 rounded-full bg-[#25543A] flex items-center justify-center font-bold text-xs">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6 relative z-0">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <MessageSquare size={48} className="text-[#25543A] mb-4 opacity-50" />
        ...
        {isLoading && <div className="text-xs text-[#25543A] animate-pulse">eNkamba réfléchit...</div>}
        <div ref={messagesEndRef} />
      </main>
      <footer className="p-6 bg-[#25543A] border-t border-white/5">
        <div className="max-w-4xl mx-auto relative">
          <input
            ...
            disabled={isLoading}
            className="w-full bg-white/5 border border-white/10 rounded-xl py-4 pl-6 pr-14 outline-none focus:border-[#25543A] transition-all disabled:opacity-50"
            placeholder="Écrivez votre message..."
          />
          <button
            onClick={() => handleSend()}
            disabled={isLoading}
            className="absolute right-2 top-2 bottom-2 aspect-square bg-[#25543A] rounded-lg flex items-center justify-center text-white disabled:opacity-50"
          >
            <Send size={18} />
          </button>
        </div>
      ...
      <div className="space-y-4">
        {aiChats.map((chat: any) => (
          <div key={chat.id} className="setting-card p-5 rounded-2xl flex items-center justify-between group">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-[#25543A]/10 rounded-xl flex items-center justify-center">
                <MessageSquare size={20} className="text-[#25543A]" />
              </div>
              <div>
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid gap-6">
        <section className="space-y-4">
          <h3 className="text-xs font-bold text-[#25543A] uppercase tracking-widest">Préférences</h3>
          <div className="setting-card p-5 rounded-2xl flex items-center justify-between">
            ...
            <div className="w-12 h-6 bg-[#25543A] rounded-full relative">
              <div className="absolute right-1 top-1 bottom-1 aspect-square bg-white rounded-full"></div>
            </div>
          </div>
        </section>
        <section className="space-y-4 mt-4">
          <h3 className="text-xs font-bold text-[#25543A] uppercase tracking-widest">Sécurité</h3>
          <div className="setting-card p-5 rounded-2xl flex items-center gap-4">
            <ShieldCheck size={20} className="text-[#25543A]" />
            <div>
...
export default function EnkambaAIModule({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex h-screen bg-[#25543A] text-white">
      <Styles />
