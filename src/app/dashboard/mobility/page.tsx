  ...
  return (
    <div className="relative">
      ...
      <Input
        ...
        className="h-12 rounded-2xl border-slate-200 bg-slate-50 pl-11 text-sm shadow-none focus-visible:ring-[#073B9A]"
      />
        ...
        <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 max-h-56 overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
          ...
          {!isLoading &&
            suggestions.map((suggestion) => (
              ...
              >
                <MapPin className="mt-0.5 h-4 w-4 flex-none text-[#073B9A]" />
...
export default function MobilityPage() {
  ...
  const startRide = () => {
    ...
    toast({
      ...
      className: 'bg-[#073B9A] text-white border-none',
    });
  };
  ...
  const rideStatusCopy: Record<RideStatus, { title: string; description: string; action: string }> = {
    ...
    in_progress: {
      ...
      description: `Direction ${rideOrder?.destination || destination}. Paiement Kenz à la fin du trajet.`,
    ...
    <div className="relative h-screen overflow-hidden bg-slate-100">
      ...
      <div className="absolute left-4 right-4 top-[calc(env(safe-area-inset-top)+1rem)] z-30 mx-auto flex max-w-xl items-center justify-between gap-3 rounded-[1.4rem] bg-white/92 p-3 shadow-xl backdrop-blur">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#073B9A] text-white">
        ...
        <Badge className="rounded-full bg-[#073B9A]/10 text-[#073B9A] hover:bg-[#073B9A]/10">Kenz</Badge>
      </div>
      ...
      {rideOrder && rideStatus !== 'idle' && (
        <div className="absolute left-4 right-4 top-[calc(env(safe-area-inset-top)+5.8rem)] z-30 mx-auto max-w-xl rounded-[1.4rem] bg-white/95 p-4 shadow-xl backdrop-blur">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-[#073B9A]">{rideOrder.id}</p>
          ...
          <div className="mt-4 flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#073B9A] text-white">
            ...
            <Button size="icon" className="rounded-full bg-[#073B9A] hover:bg-[#073B9A]">
          ...
          {rideStatus === 'searching' && (
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100">
              <div className="h-full w-2/3 animate-pulse rounded-full bg-[#073B9A]" />
            </div>
          )}

          {(rideStatus === 'accepted' || rideStatus === 'arriving') && (
            <Button onClick={startRide} className="mt-4 h-11 w-full rounded-full bg-[#073B9A] font-bold hover:bg-[#073B9A]">
      ...
      <button
        ...
        className="absolute z-20 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white text-[#073B9A] shadow-xl ring-4 ring-[#073B9A]/20"
        style={markerPosition(userPosition, mapViewCenter, mapRadius)}
      >
        <span className="absolute h-12 w-12 animate-ping rounded-full bg-[#073B9A]/20" />
        <Navigation className="relative h-5 w-5 fill-[#073B9A]" />
      </button>

      {pickupPoint && (
        <div
          className="absolute z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full bg-[#073B9A] px-2.5 py-1.5 text-xs font-semibold text-white shadow-lg ring-2 ring-white"
      ...
      {destinationPoint && (
        <div
          className="absolute z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full bg-[#F51B2B] px-2.5 py-1.5 text-xs font-semibold text-white shadow-lg ring-2 ring-white"
      ...
      {rideOptions.map((ride) => (
        <button
          ...
          className={`absolute z-20 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full shadow-lg ring-2 ring-white transition ${
            selectedRide === ride.id ? 'h-11 w-11 bg-[#073B9A] text-white' : 'h-9 w-9 bg-white text-[#073B9A]'
          }`}
      ...
      <div className="absolute right-4 top-28 z-30 flex flex-col gap-2">
        ...
        <Button size="icon" className="rounded-full bg-white text-[#073B9A] shadow-lg hover:bg-white" onClick={recenterMap}>
      ...
      <section className="absolute bottom-0 left-0 right-0 z-40 rounded-t-[2rem] bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-[0_-16px_40px_rgba(15,23,42,0.18)]">
        <div className="mx-auto max-w-xl space-y-4">
          ...
          {rideOrder && rideStatus !== 'idle' ? (
            <div className="space-y-3">
              <div className="grid grid-cols-[20px_1fr] gap-x-3 gap-y-2 rounded-2xl bg-slate-50 p-4">
                ...
                <LocateFixed className="mt-0.5 h-4 w-4 text-[#F51B2B]" />
                <p className="truncate text-sm font-semibold text-slate-900">{rideOrder.destination}</p>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-[#073B9A]/10 px-4 py-3">
                <div>
                ...
                <p className="text-sm font-extrabold text-[#073B9A]">{rideOrder.ride.eta}</p>
              </div>
          ...
          <div className="space-y-2">
            ...
            />
            <button type="button" onClick={useCurrentPosition} className="text-xs font-semibold text-[#073B9A]">
            ...
            {rideOptions.map((option) => (
              <button
                ...
                className={`min-w-[145px] rounded-2xl border p-3 text-left transition ${
                  selectedRide === option.id ? 'border-[#073B9A] bg-[#073B9A]/10' : 'border-slate-200 bg-white'
                }`}
              >
          ...
          <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Paiement Kenz disponible</p>
              </div>
            </div>
            <p className="text-sm font-extrabold text-[#073B9A]">{selectedRideOption.eta}</p>
          </div>

          <Button onClick={requestRide} className="h-12 w-full rounded-full bg-[#073B9A] text-base font-bold hover:bg-[#073B9A]">
