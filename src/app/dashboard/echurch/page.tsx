...
const statusClassName = (status: ChurchAccount['status']) => {
  if (status === 'validated') return 'border-[#0A8B46]/15 bg-[#0A8B46]/10 text-[#0A8B46]';
  if (status === 'rejected' || status === 'suspended') return 'border-red-100 bg-red-50 text-red-700';
  return 'border-[#FF8A00]/20 bg-[#FF8A00]/10 text-[#B15F00]';
};
...
export default function EChurchPage() {
  ...
  const createAccount = async () => {
    ...
    try {
      ...
      const data = {
        ownerId: user.uid,
        ownerName: user.displayName || user.email || 'Administrateur eNkamba',
      ...
      toast({ title: 'Compte eChurch soumis', description: 'La vérification eNkamba est lancée. Le QR institutionnel sera activé après validation.' });
    } catch (error) {
  ...
  const downloadInstitutionalQr = async () => {
    ...
    const result = await createBrandedQRCodeDataUrl({
      ...
      details: [{ label: 'Paiement', value: 'eNkamba Church Pay' }],
  ...
  const downloadReport = () => {
    ...
    report.text('eNkamba Church Pay', 15, 17);
    ...
    report.text('Document généré par eNkamba Church Pay. Les données financières restent traçables dans le journal du compte.', 15, 286, { maxWidth: 180 });
    report.save(`rapport-echurch-${slugifyChurchName(account.name)}.pdf`);
  };

  if (isAuthLoading || isAccountLoading) {
    return (
      <><DashboardHeader /><main className="min-h-screen bg-[#F7FAF8] pt-24"><div className="flex min-h-[60vh] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div></main></>
    );
  }

  if (!user) {
    return (
      <><DashboardHeader /><main className="min-h-screen bg-[#F7FAF8] px-4 pb-8 pt-24"><section className="mx-auto max-w-lg rounded-3xl bg-white p-7 text-center shadow-sm"><ChurchMark className="mx-auto h-16 w-16" /><h1 className="mt-5 text-2xl font-black text-slate-950">eNkamba Church Pay</h1><p className="mt-2 text-sm leading-6 text-slate-600">Connectez-vous pour créer ou administrer votre compte Église.</p><Button asChild className="mt-6 bg-primary hover:bg-primary"><Link href="/login">Se connecter</Link></Button></section></main></>
    );
  }
  ...
  if (!account) {
    return (
      <>
        <DashboardHeader />
        <main className="min-h-screen bg-[#F7FAF8] px-3 pb-10 pt-24 sm:px-5">
          <section className="mx-auto max-w-3xl overflow-hidden rounded-[26px] bg-white shadow-sm ring-1 ring-primary/10">
            <div className="relative overflow-hidden bg-primary px-5 pb-20 pt-6 text-white sm:px-8">
              ...
              <div className="relative flex items-start justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[0.18em] text-white/75">eNkamba Pay</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">eChurch</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/84">Centralisez les dons, dîmes, offrandes et campagnes de votre communauté avec eNkamba Pay.</p></div><ChurchMark className="h-16 w-16 shrink-0" /></div>
            </div>
...
function ChurchTab({ value, label, icon: Icon, tone = 'green' }: { value: DashboardTab; label: string; icon: typeof BarChart3; tone?: 'green' | 'orange' }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute bottom-1.5 h-1 w-1 rounded-full bg-[#FF8A00] opacity-0 transition-opacity group-data-[state=active]:opacity-100 sm:bottom-1" />
    </TabsTrigger>
  );
}
function Metric({ icon: Icon, label, value, tone }: { icon: typeof WalletCards; label: string; value: string; tone: 'green' | 'orange' }) { const color = tone === 'orange' ? 'bg-[#FF8A00]/10 text-[#D16F00]' : 'bg-primary/10 text-primary'; return <Card className="rounded-2xl border-0 shadow-sm"><CardContent className="p-4"><div className={`flex h-9 w-9 items-center justify-center rounded-xl ${color}`}><Icon className="h-5 w-5" /></div><p className="mt-3 text-xs font-bold text-slate-500">{label}</p><p className="mt-1 truncate text-lg font-black text-slate-950">{value}</p></CardContent></Card>; }
function TransactionRow({ transaction, detailed = false }: { transaction: ChurchTransaction; detailed?: boolean }) { return <div className="flex items-center gap-3 py-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><ReceiptText className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-slate-900">{transaction.categoryLabel}</p><p className="mt-0.5 truncate text-xs text-slate-500">{transaction.isAnonymous ? 'Contribution anonyme' : transaction.contributorName || 'Fidèle eNkamba'}{transaction.parishName ? ` · ${transaction.parishName}` : ''}</p>{detailed && <p className="mt-1 text-[11px] font-medium text-slate-400">{transaction.reference} · {formatDate(transaction.paidAt || transaction.createdAt, true)}</p>}</div><div className="text-right"><p className="text-sm font-black text-primary">+{formatChurchCurrency(transaction.amount)}</p><p className="mt-0.5 text-[11px] text-slate-500">{formatDate(transaction.paidAt || transaction.createdAt)}</p></div></div>; }
