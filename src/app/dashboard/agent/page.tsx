...
const AgentGatewayIcon = ({ className = 'h-6 w-6' }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <circle cx="24" cy="24" r="18" fill="#32BB78" />
    ...
    <circle cx="33" cy="35" r="4" fill="#173f2b" />
  </svg>
);
      ...
      <div className="min-h-screen bg-[#f7faf8]">
        <div className="container mx-auto flex min-h-[70vh] max-w-4xl items-center justify-center p-4">
          <div className="text-center">
            <Loader2 className="mx-auto mb-3 h-10 w-10 animate-spin text-[#32BB78]" />
      ...
      <div className="container mx-auto max-w-4xl p-3 sm:p-4">
        <Card className="overflow-hidden border-0 bg-white shadow-sm">
          <CardContent className="p-0">
            <div className="relative overflow-hidden bg-gradient-to-br from-[#32BB78] to-[#21945e] px-4 py-6 text-white">
            ...
            <div className="grid gap-4 p-4 md:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-2xl border border-[#32BB78]/10 bg-[#f7faf8] p-4">
                <div className="mb-4 flex items-center gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#32BB78]/10">
                    <StatusIcon className={`h-5 w-5 ${view.tone === 'red' ? 'text-red-600' : view.tone === 'orange' ? 'text-[#FFB545]' : 'text-[#32BB78]'}`} />
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">Destination</p>
                    <p className="font-bold text-[#173f2b]">{destination}</p>
                  </div>
                </div>

                <div className="h-2 overflow-hidden rounded-full bg-[#32BB78]/10">
                  <div
                    className="h-full rounded-full bg-[#32BB78] transition-all duration-700"
                    style={{ width: `${Math.min(100, Math.max(8, progress))}%` }}
                  />
                </div>
                <p className="mt-3 text-sm text-muted-foreground">
                  Redirection automatique dans <span className="font-black text-[#173f2b]">{remainingSeconds}</span> seconde{remainingSeconds > 1 ? 's' : ''}.
                </p>

                <Button
                  className="mt-4 h-11 w-full rounded-xl bg-[#32BB78] font-bold hover:bg-[#299c63]"
              ...
              <div className="space-y-3">
                <div className="rounded-2xl border border-[#32BB78]/10 bg-white p-4 shadow-sm">
                  <ShieldCheck className="mb-2 h-5 w-5 text-[#32BB78]" />
                  <p className="text-sm font-bold text-[#173f2b]">Statut vérifié</p>
                ...
                <div className="rounded-2xl border border-[#32BB78]/10 bg-white p-4 shadow-sm">
                  {hasApprovedPaymentBusiness ? (
                    <>
                      <Building2 className="mb-2 h-5 w-5 text-[#32BB78]" />
                      <p className="text-sm font-bold text-[#173f2b]">Compte entreprise paiement lié</p>
                      <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    ...
                    <>
                      ...
                      <p className="text-sm font-bold text-[#173f2b]">Compte agent dédié</p>
                      <p className="mt-1 text-xs leading-5 text-muted-foreground">
