    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Rejoignez Kenz',
          text: 'Rejoignez Kenz et recevez des bonus de parrainage!',
          url: referralLink,
        });
      } catch (error) {
        ...
        {isLoading ? (
          <Card>
            <CardContent className="pt-6 flex justify-center">
              <Loader2 className="h-8 w-8 animate-spin text-[#073B9A]" />
            </CardContent>
          </Card>
        ) : (
          <>
            ...
            {referralLink && (
              <Card className="border-[#073B9A]/20 bg-gradient-to-br from-[#073B9A]/10 to-[#073B9A]/5">
                ...
                <CardContent className="space-y-4">
                  <div className="bg-white p-4 rounded-lg border border-[#073B9A]/20 break-all font-mono text-sm">
                    {referralLink}
                  </div>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold text-[#073B9A]">{stats.totalReferrals}</p>
                      </div>
                      <div className="p-3 rounded-full bg-[#073B9A]/20">
                        <Users className="w-6 h-6 text-[#073B9A]" />
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold text-[#073B9A]">
                          {stats.totalEarnings.toLocaleString('fr-FR')} CDF
                        </p>
                      </div>
                      <div className="p-3 rounded-full bg-[#073B9A]/20">
                        <Gift className="w-6 h-6 text-[#073B9A]" />
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex gap-3">
                    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[#073B9A] text-white flex items-center justify-center font-bold">
                      1
                    </div>
                    <div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-[#073B9A]" />
              ...
              <CardContent>
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    <span className="text-[#073B9A]">✓</span>
