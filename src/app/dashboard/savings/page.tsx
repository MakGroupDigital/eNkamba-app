...
const SavingsVaultIcon = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <rect x="8" y="14" width="32" height="25" rx="8" fill="#009058" />
    <path d="M14 20h20" stroke="white" strokeWidth="3" strokeLinecap="round" opacity="0.85" />
    <circle cx="24" cy="28" r="6" fill="#009058" opacity="0.9" />
    <circle cx="24" cy="28" r="2" fill="#FFA500" />
    <path d="M17 14v-2a7 7 0 0 1 14 0v2" stroke="#009058" strokeWidth="3" strokeLinecap="round" opacity="0.4" />
  </svg>
);

const GoalSeedIcon = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <path d="M24 39V22" stroke="#009058" strokeWidth="3" strokeLinecap="round" />
    <path d="M24 24c-8 0-13-5-13-12 8 0 13 5 13 12Z" fill="#009058" />
    <path d="M24 27c8 0 13-5 13-12-8 0-13 5-13 12Z" fill="#FFA500" />
    <rect x="13" y="37" width="22" height="4" rx="2" fill="#009058" opacity="0.35" />
  </svg>
);

const AutoSaveIcon = ({ className = "h-6 w-6" }: { className?: string }) => (
  <svg viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true">
    <rect x="9" y="10" width="30" height="28" rx="8" fill="#009058" />
    <path d="M16 25h13" stroke="white" strokeWidth="3" strokeLinecap="round" />
    <path d="M26 18l7 7-7 7" stroke="#009058" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
    <circle cx="17" cy="18" r="3" fill="#FFA500" />
  </svg>
);
    ...
    <div className="container mx-auto max-w-4xl p-3 space-y-4 animate-in fade-in duration-500 sm:p-4">
      ...
      <header className="overflow-hidden rounded-2xl bg-gradient-to-br from-[#009058] to-[#009058] p-4 text-white shadow-lg shadow-[#009058]/20">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button onClick={() => setShowCreateDialog(true)} className="h-10 shrink-0 rounded-xl bg-white px-3 text-xs font-bold text-[#009058] hover:bg-white/90 sm:text-sm">
      ...
      <Card className="overflow-hidden border-0 bg-white shadow-sm">
        <CardHeader className="border-b border-[#009058]/10 px-4 py-3">
          <CardTitle className="font-headline flex items-center gap-2 text-lg text-[#009058]">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#009058]/10">
              <PiggyBank className="h-5 w-5 text-[#009058]" />
            </span>
            Épargne Totale
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="grid gap-3 p-4 sm:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl bg-[#009058] p-4 text-white shadow-sm shadow-[#009058]/20">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-1">
            <div className="rounded-2xl border border-[#009058]/10 bg-[#f7faf8] p-3">
              <WalletCards className="mb-2 h-5 w-5 text-[#009058]" />
              <p className="text-xs font-bold text-[#009058]">Portefeuille lié</p>
              <p className="mt-1 text-sm font-black text-[#009058]">{formatCurrency(walletBalance, 'CDF')}</p>
            </div>
            <div className="rounded-2xl border border-[#009058]/10 bg-[#f7faf8] p-3">
              <ShieldCheck className="mb-2 h-5 w-5 text-[#FFA500]" />
              <p className="text-xs font-bold text-[#009058]">Débits planifiés</p>
      ...
      {/* No Goals State */}
      {goals.length === 0 && (
        <Card className="border-2 border-dashed border-[#009058]/30 bg-white">
          <CardContent className="space-y-4 p-10 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#009058]/10">
              <GoalSeedIcon className="h-10 w-10" />
            </div>
            <div>
              <h3 className="font-headline mb-2 text-lg font-bold text-[#009058]">Aucun objectif d'épargne</h3>
              <p className="mb-4 text-sm text-muted-foreground">Créez votre premier objectif d'épargne pour commencer à économiser.</p>
            </div>
            <Button onClick={() => setShowCreateDialog(true)} className="rounded-xl bg-[#009058] font-bold hover:bg-[#009058]">
      ...
      {goals.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-headline text-lg font-bold text-[#009058]">Mes Objectifs</h2>
            <div className="rounded-full bg-[#009058]/10 px-3 py-1 text-xs font-bold text-[#009058]">
          ...
          {goals.map((goal) => {
            ...
            return (
              <Card key={goal.id} className={`overflow-hidden border-[#009058]/10 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${isCompleted ? 'border-[#009058]/30 bg-[#009058]/5' : ''}`}>
                <CardContent className="p-4">
                  <div className="space-y-4">
                    {/* Header */}
                    <div className="flex items-start justify-between">
                      <div className="flex flex-1 items-start gap-3">
                        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#009058]/10 text-3xl ring-1 ring-[#009058]/10">{goal.icon}</div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-headline truncate font-bold text-[#009058]">{goal.name}</h3>
                            {isCompleted && (
                              <CheckCircle2 className="h-5 w-5 shrink-0 text-[#009058]" />
                            )}
                            {goal.status === 'paused' && (
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1 rounded-full bg-[#009058]/10 px-2 py-1 font-semibold text-[#009058]">
                              <Zap className="h-3.5 w-3.5" />
                              {goal.frequency === 'daily' ? 'Quotidien' : goal.frequency === 'weekly' ? 'Hebdomadaire' : 'Mensuel'}
                            </span>
                            <span className="rounded-full bg-[#f7faf8] px-2 py-1 font-semibold text-[#009058]">{formatCurrency(goal.frequencyAmount, goal.currency)}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-black text-[#009058]">{progress.toFixed(0)}%</p>
                    ...
                    <div className="space-y-2">
                      <Progress value={progress} className="h-3 rounded-full bg-[#009058]/10" />
                    ...
                    {/* Status Alert */}
                    {isCompleted && (
                      <Alert className="rounded-2xl border-[#009058]/20 bg-[#009058]/5">
                        <CheckCircle2 className="h-4 w-4 text-[#009058]" />
                        <AlertTitle className="text-[#009058]">Objectif atteint!</AlertTitle>
                        <AlertDescription className="text-[#009058]">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <Button
                        ...
                        className="h-9 flex-1 rounded-xl border-[#009058]/20 text-[#009058] hover:bg-[#009058]/5"
                      >
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-[#009058]">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#009058]/10">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="rounded-xl border-[#009058]/20 focus-visible:ring-[#009058]"
              />
            </div>
                ...
                {GOAL_ICONS.map((icon) => (
                  <button
                    ...
                    className={`rounded-xl border-2 p-2 text-2xl transition-all ${
                      formData.icon === icon
                        ? 'border-[#009058] bg-[#009058]/10'
                        : 'border-muted hover:border-[#009058]/50'
                    }`}
                  >
                    {icon}
                  </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.currency} onValueChange={(value: any) => setFormData({ ...formData, currency: value })}>
                  <SelectTrigger className="rounded-xl border-[#009058]/20">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <Alert className="rounded-2xl border-[#009058]/20 bg-[#009058]/5">
              ...
              <AlertTitle className="text-[#009058]">Contributions automatiques</AlertTitle>
              <AlertDescription>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateGoal} disabled={isProcessing} className="bg-[#009058] hover:bg-[#009058]">
      ...
      <Dialog open={showAddFundsDialog} onOpenChange={setShowAddFundsDialog}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-[#009058]">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#009058]/10">
                <Plus className="h-5 w-5 text-[#009058]" />
              </span>
              Ajouter des fonds
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2 rounded-2xl bg-[#f7faf8] p-4">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-[#009058]">{formatCurrency(walletBalance, 'CDF')}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Solde objectif:</span>
                <span className="font-bold text-[#009058]">{formatCurrency(selectedGoal?.currentAmount || 0, selectedGoal?.currency || 'CDF')}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="rounded-xl border-[#009058]/20 text-lg font-bold focus-visible:ring-[#009058]"
              />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddFunds} disabled={isProcessing} className="bg-[#009058] hover:bg-[#009058]">
      ...
      <Dialog open={showWithdrawDialog} onOpenChange={setShowWithdrawDialog}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-[#009058]">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#009058]/10">
                <ArrowRight className="h-5 w-5 text-[#009058]" />
              </span>
              Retirer des fonds
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleWithdraw} disabled={isProcessing} className="bg-[#009058] hover:bg-[#009058]">
