...
export default function ChurchPaymentPage() {
  ...
  const beginPayment = () => {
    ...
    if (amount > balance) {
      toast({ variant: 'destructive', title: 'Solde insuffisant', description: `Votre wallet eNkamba contient ${formatChurchCurrency(balance)}.` });
      return;
    }
    setShowPin(true);
  };
  ...
  const downloadReceipt = () => {
    ...
    pdf.setFontSize(17);
    pdf.text('eNkamba Church Pay', 15, 17);
    pdf.setFontSize(10);
    ...
    const content = [
      ...
      `Canal : Wallet eNkamba Pay`,
      `Statut : Confirmé`,
      `Date : ${new Intl.DateTimeFormat('fr-CD', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date())}`,
      '',
      'Ce reçu confirme la réception de votre contribution. Il peut être vérifié à partir de sa référence dans eNkamba Church Pay.',
    ];
    pdf.text(content, 15, 64, { maxWidth: 180, lineHeightFactor: 1.7 });
    pdf.setFontSize(8);
    pdf.setTextColor(100, 116, 139);
    pdf.text('eNkamba Church Pay - Gestion digitale des dons, dîmes, offrandes et finances de paroisses.', 15, 287, { maxWidth: 180 });
    pdf.save(`recu-${receipt.reference}.pdf`);
  };

  if (isLoading || isAuthLoading) {
    return <main className="min-h-screen bg-[#F7FAF8]"><div className="flex min-h-screen items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div></main>;
  }

  if (!account) {
    return <main className="flex min-h-screen items-center justify-center bg-[#F7FAF8] p-5"><Card className="w-full max-w-md rounded-3xl border-0 shadow-sm"><CardContent className="p-7 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary"><QrCode className="h-8 w-8" /></span><h1 className="mt-5 text-xl font-black text-slate-950">Lien eChurch introuvable</h1><p className="mt-2 text-sm leading-6 text-slate-600">Ce QR ou lien de contribution n’est plus disponible.</p><Button asChild variant="outline" className="mt-6"><Link href="/dashboard/mbongo-dashboard"><ArrowLeft className="mr-2 h-4 w-4" />Retour à eNkamba Pay</Link></Button></CardContent></Card></main>;
  }

  if (account.status !== 'validated') {
    return <main className="flex min-h-screen items-center justify-center bg-[#F7FAF8] p-5"><Card className="w-full max-w-md rounded-3xl border-0 shadow-sm"><CardContent className="p-7 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#FF8A00]/10 text-[#D16F00]"><ShieldCheck className="h-8 w-8" /></span><h1 className="mt-5 text-xl font-black text-slate-950">Collecte indisponible</h1><p className="mt-2 text-sm leading-6 text-slate-600">Le compte de {account.name} est en cours de vérification. Réessayez après son activation officielle.</p></CardContent></Card></main>;
  }

  if (receipt) {
    return <main className="min-h-screen bg-[#F7FAF8] p-4 sm:p-6"><section className="mx-auto max-w-md overflow-hidden rounded-[28px] bg-white shadow-xl shadow-primary/10"><div className="bg-primary px-6 pb-11 pt-6 text-center text-white"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white/15"><CheckCircle2 className="h-8 w-8" /></div><p className="mt-4 text-xl font-black">Contribution confirmée</p><p className="mt-2 text-sm text-white/78">Merci pour votre soutien à {account.name}.</p></div><div className="-mt-6 px-5 pb-6"><div className="rounded-2xl bg-white p-4 text-center shadow-lg shadow-black/10"><p className="text-xs font-bold text-slate-500">Montant reçu</p><p className="mt-1 text-3xl font-black text-primary">{formatChurchCurrency(amount)}</p><Badge className="mt-3 border border-primary/15 bg-primary/10 text-primary">{selectedCategory.label}</Badge></div><div className="mt-5 space-y-3 rounded-2xl bg-slate-50 p-4 text-sm"><ReceiptLine label="Référence" value={receipt.reference} /><ReceiptLine label="Paroisse" value={selectedParish?.name || 'Collecte générale'} /><ReceiptLine label="Canal" value="Wallet eNkamba Pay" /><ReceiptLine label="Statut" value="Confirmé" /></div><Button className="mt-5 w-full bg-primary hover:bg-primary" onClick={downloadReceipt}><ReceiptText className="mr-2 h-4 w-4" />Télécharger le reçu PDF</Button><Button asChild variant="ghost" className="mt-2 w-full text-primary"><Link href="/dashboard/mbongo-dashboard">Retour à eNkamba Pay</Link></Button></div></section></main>;
  }

  return (
    <main className="min-h-screen bg-[#F7FAF8] pb-8">
      <section className="relative overflow-hidden bg-primary px-5 pb-16 pt-6 text-white"><div className="absolute -right-20 -top-14 h-48 w-48 rounded-full bg-white/10" /><div className="relative mx-auto flex max-w-2xl items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-2xl bg-white p-1">{account.logoUrl ? <img src={account.logoUrl} alt="Logo" className="h-full w-full rounded-xl object-cover" /> : <EChurchIcon size={48} className="h-full w-full" />}</div><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.14em] text-white/70">eNkamba Church Pay</p><h1 className="mt-1 truncate text-lg font-black">{account.name}</h1></div></div><Badge className="border border-white/20 bg-white/10 text-white"><BadgeCheck className="mr-1 h-3.5 w-3.5" />Vérifié</Badge></div></section>
      <section className="relative z-10 mx-auto -mt-9 max-w-2xl px-3 sm:px-5"><Card className="rounded-[24px] border-0 shadow-xl shadow-black/10"><CardContent className="p-4 sm:p-6"><div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><HeartHandshake className="h-5 w-5" /></span><div><p className="text-sm font-black text-slate-950">Contribuer en toute confiance</p><p className="mt-0.5 text-xs text-slate-500">Votre contribution est enregistrée et un reçu est généré après confirmation.</p></div></div>{contextQr && <div className="mt-4 flex items-center gap-3 rounded-xl border border-primary/10 bg-primary/[0.04] p-3"><QrCode className="h-5 w-5 text-primary" /><p className="text-sm font-bold text-slate-700">{contextQr.campaignName || contextQr.parishName || contextQr.categoryLabel || 'Contribution ciblée'}</p></div>}
        ...
        {user && <div className="mt-4 flex items-center justify-between rounded-xl border border-primary/10 bg-primary/[0.04] p-3"><span className="flex items-center gap-2 text-sm font-bold text-slate-700"><WalletCards className="h-4 w-4 text-primary" />Solde eNkamba Pay</span><span className="font-black text-primary">{formatChurchCurrency(balance)}</span></div>}
        <Button className="mt-5 h-12 w-full bg-primary text-base font-black hover:bg-primary" disabled={!canPay && Boolean(user)} onClick={beginPayment}>{isProcessing ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <WalletCards className="mr-2 h-5 w-5" />}{user ? `Payer ${amount > 0 ? formatChurchCurrency(amount) : ''}` : 'Se connecter pour contribuer'}</Button><div className="mt-4 flex items-start gap-2 text-xs leading-5 text-slate-500"><LockKeyhole className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />Paiement sécurisé par eNkamba Pay. Le PIN est demandé avant toute confirmation.</div></CardContent></Card></section>
      <footer className="mx-auto mt-6 flex max-w-2xl items-center justify-center gap-2 px-5 text-center text-xs text-slate-500"><ShieldCheck className="h-4 w-4 text-primary" />Contribution traçable · Reçu électronique · eNkamba Church Pay</footer>
