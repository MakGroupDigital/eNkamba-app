...
export function PaymentDashboard({ businessUser }: PaymentDashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(50,187,120,0.14),transparent_34%),linear-gradient(180deg,rgba(50,187,120,0.05)_0%,rgba(50,187,120,0.08)_54%,rgba(50,187,120,0.04)_100%)] pb-24 text-foreground">
      <div className="sticky top-0 z-30 rounded-b-[32px] bg-gradient-to-r from-[#0A8B46] via-[#0A8B46] to-[#0A8B46] px-4 pb-4 pt-[calc(env(safe-area-inset-top)+0.75rem)] text-white shadow-lg shadow-[#0A8B46]/20">
      ...
      <div className="mx-auto max-w-5xl space-y-5 px-4 py-5">
        <section className="overflow-hidden rounded-3xl border border-[#0A8B46] bg-white shadow-sm">
            ...
            {tabs.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex min-w-fit items-center gap-2 rounded-2xl px-4 py-3 text-sm font-bold transition ${
                  activeTab === id
                    ? 'bg-[#0A8B46] text-white shadow-md shadow-[#0A8B46]/20'
      ...
      <Panel title="Statut API" subtitle="Préparation du trafic technique Mbongo.">
        <div className="grid gap-3">
          {[
            ...
            <div key={label} className="flex items-center justify-between rounded-2xl bg-[#0A8B46] px-4 py-3">
    ...
    <Panel title="Tokens d’accès" subtitle="Créez des tokens limités par usage, durée et module." action="+ Nouveau token">
      <div className="grid gap-3 md:grid-cols-3">
        {[
          ...
          <div key={token.title} className="rounded-3xl border border-[#0A8B46] bg-[#0A8B46] p-4">
            ...
            <span className="mt-4 inline-flex rounded-full bg-white px-3 py-1 text-xs font-bold text-[#0A8B46]">
              {token.status}
            </span>
          </div>
        ))}
...
function PaymentGeneration() {
  ...
  return (
    <Panel title="Génération" subtitle="Génération locale de prévisualisation. La clé réelle doit être créée côté serveur sécurisé.">
      <div className="rounded-3xl bg-[#0A8B46] p-5 text-white">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          <button onClick={generateKey} className="rounded-2xl bg-[#0A8B46] px-4 py-3 text-sm font-bold text-white">Générer</button>
          <button className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-[#0A8B46]">Copier</button>
    ...
    <Panel title="Intégration" subtitle="Points d’intégration recommandés pour vos apps, sites et boutiques.">
      <div className="grid gap-3 md:grid-cols-2">
        {[
          ...
          <div key={title} className="rounded-3xl border border-[#0A8B46] bg-white p-4">
    ...
    <Panel title="Documentation" subtitle="Guides rapides pour brancher Mbongo dans un produit.">
      <div className="space-y-3">
        {[
          ...
          <button key={title} className="flex w-full items-center justify-between gap-4 rounded-2xl bg-[#0A8B46] px-4 py-4 text-left">
            <span>
...
function AgentBalance() {
  return (
    <div className="space-y-5">
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-3xl bg-gradient-to-br from-[#0A8B46] to-[#0A8B46] p-6 text-white">
      ...
      <Panel title="Relevé du jour" subtitle="Synthèse des dépôts, retraits et net business.">
        {['Dépôts', 'Retraits', 'Net du jour'].map((item) => (
          <div key={item} className="mb-3 flex items-center justify-between rounded-2xl bg-[#0A8B46] px-4 py-3">
...
function Panel({
  ...
  return (
    <section className="rounded-3xl border border-[#0A8B46] bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {action && (
          <button className="rounded-2xl bg-[#0A8B46] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#0A8B46]">
            {action}
          </button>
        )}
      </div>
...
function ApiCredential({ label, value, sensitive = false }: { label: string; value: string; sensitive?: boolean }) {
  return (
    <div className="mb-3 rounded-2xl border border-[#0A8B46] bg-[#0A8B46] p-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <button className="rounded-xl bg-white px-4 py-3 text-sm font-bold text-[#0A8B46] ring-1 ring-[#0A8B46]">Copier</button>
      </div>
    </div>
  );
}
...
function PaymentMetricCard({ stat }: { stat: { label: string; value: string; icon: React.ComponentType<any>; color: string } }) {
  ...
  const colorClasses = {
    green: 'bg-[#0A8B46] text-[#0A8B46] border-[#0A8B46]',
    blue: 'bg-[#eff6ff] text-[#1d4ed8] border-[#bfdbfe]',
    emerald: 'bg-[#0A8B46] text-[#0A8B46] border-[#0A8B46]',
...
function EmptyPaymentState({ icon: Icon, text }: { icon: React.ComponentType<any>; text: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-[#0A8B46] bg-[#0A8B46] px-5 py-12 text-center">
      <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-3xl bg-white shadow-sm">
        <Icon className="h-12 w-12 text-[#0A8B46]" size={48} />
      </div>
